...
const createDB = async () => {
  ...
  db = {
    getAllEntities: async (entity) => {
      ...
      const [rows] = await pool.query(`SELECT id, title, url, category FROM \`${entity}\``);
      return rows;
    },
    ...
    addEntitiy: async (entity, newItem) => {
      ...
      const [result] = await pool.execute(sql, [newItem.title, newItem.url, newItem.category]);
    ...
    updateEntitiy: async (entity, id, newItem) => {
      ...
      const fields = [];
      const values = [];
      if (newItem.title !== undefined) { fields.push("title = ?"); values.push(newItem.title); }
      if (newItem.url !== undefined) { fields.push("url = ?"); values.push(newItem.url); }
      if (newItem.category !== undefined) { fields.push("category = ?"); values.push(newItem.category); }
