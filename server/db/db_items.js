  ...
  db = {
      ...
      try {
        const [result] = await conn.execute(
          `INSERT INTO items (category, name, image, url, rarity, description, time, base_value, behavior, bait, family)
           VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
          [data.category, data.name, data.image, data.url, data.rarity ?? null,
           data.description ?? null, data.time ?? null, data.baseValue ?? null,
           data.behavior ?? null, data.bait ?? null, data.family ?? null]
        );
    ...
    updateItem: async (id, data) => {
      ...
      try {
        await conn.execute(
          ...
          [data.name, data.image, data.url, data.rarity ?? null,
           data.description ?? null, data.time ?? null, data.baseValue ?? null,
           data.behavior ?? null, data.bait ?? null, data.family ?? null, id]
        );
    ...
    deleteItem: async (id) => {
      ...
      try {
        ...
        await conn.execute("DELETE FROM item_needed_for_link WHERE item_id = ?", [id]);
        await conn.execute("DELETE FROM item_how_to_obtain_link WHERE item_id = ?", [id]);
        const [result] = await conn.execute("DELETE FROM items WHERE id = ?", [id]);
        await conn.commit();
...
const insertLinks = async (conn, id, data) => {
  for (const loc of data.location ?? []) {
    const [[entity]] = await conn.query("SELECT id FROM location_entity WHERE title = ?", [loc.title]);
    if (entity) await conn.execute("INSERT INTO item_location_link (item_id, location_id) VALUES (?, ?)", [id, entity.id]);
  }
  for (const nf of data.neededFor ?? []) {
    const [[entity]] = await conn.query("SELECT id FROM needed_for_entity WHERE title = ?", [nf.title]);
    if (entity) await conn.execute("INSERT INTO item_needed_for_link (item_id, needed_for_id) VALUES (?, ?)", [id, entity.id]);
  }
  for (const hto of data.howToObtain ?? []) {
    const [[entity]] = await conn.query("SELECT id FROM how_to_obtain_entity WHERE title = ?", [hto.title]);
    if (entity) await conn.execute("INSERT INTO item_how_to_obtain_link (item_id, how_to_obtain_id) VALUES (?, ?)", [id, entity.id]);
  }
};
