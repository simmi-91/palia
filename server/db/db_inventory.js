...
const createDB = async () => {
    ...
    db = {
      ...
      `;
            const [result] = await pool.execute(sql, [profileId, category, itemId, amount]);
            return { success: true, count: result.affectedRows - result.changedRows };
        },
      ...
      `;
        ...
        getTradeable: async (profileId) => {
            const [rows] = await pool.query(
                `SELECT ui.user_id, u.given_name, ui.category, ui.item_id,
